...
--
-- Host: 127.0.0.1
-- Generation Time: Jan 28, 2023 at 05:14 PM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `getChildren` (IN `inp` VARCHAR(50))   SELECT cname,cdob,cyoe,cstory,cclass FROM children WHERE cid=inp$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `getdonation` ()   SELECT * FROM DONATION$$
...
(9, 'Sagar', '2006-06-14', 2009, 2, 'Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.', '', 0),
...
(17, 'RAM', '2010-07-13', 2017, 1, 'nothing', '', 0),
(19, 'SHASHANK', '2019-12-19', 2018, 4, 'nothing', '', 0),
(22, 'PARAM', '2023-01-03', 2022, 4, '', '', 0),
(23, 'RAJ', '2006-12-06', 2019, 5, 'nothing', '', 0),
(24, 'SHAMHU', '2019-11-18', 2019, 7, 'nothing', '', 0);

--
-- Triggers `children`
--
DELIMITER $$
CREATE TRIGGER `InsertLog` AFTER INSERT ON `children` FOR EACH ROW INSERT INTO tlogs VALUES(null, NEW.cid, 'Inserted', NOW())
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `Updatelog` AFTER UPDATE ON `children` FOR EACH ROW INSERT INTO tlogs VALUES(null, NEW.cid, 'Updated', NOW())
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `cname` BEFORE INSERT ON `children` FOR EACH ROW SET NEW.cname=UPPER(NEW.cname)
$$
DELIMITER ;
...
(3, 'hari', '356a192b7913b04c54574d18c28d46e6395428ab', '2023-01-16 16:04:29', NULL, NULL, NULL, NULL, NULL, NULL, NULL),
(4, 'jeevika', '51eac6b471a284d3341d8c0c63d0f1a286262a18', '2023-01-18 16:53:24', NULL, NULL, NULL, NULL, NULL, NULL, NULL);
...
(2, 'Newly born children', 'Caring for a newborn is not an impossible feat, but you\'ll need to engender a firm commitment to the task and an illimitable supply of patience and love in preparation for your baby\'s arrival.For parents who have maintained reasonable expectations of the infant stage and are ready to adapt, a newborn baby will be a consummate blessing.', 'August'),
(3, '', '', '');
...
INSERT INTO `tlogs` (`id`, `cid`, `action`, `cdate`) VALUES
(1, 22, 'Inserted', '2023-01-18 00:13:12'),
(2, 23, 'Inserted', '2023-01-18 00:14:31'),
(3, 18, 'Updated', '2023-01-18 00:19:10'),
(6, 24, 'Inserted', '2023-01-18 16:59:26');
...
--
ALTER TABLE `children`
  MODIFY `cid` int(50) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=25;
...
--
ALTER TABLE `newsletter`
  MODIFY `n_id` int(3) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;
